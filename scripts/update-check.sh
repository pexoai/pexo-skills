#!/usr/bin/env bash
# pexo-skills update-check — opt-in version check.
# Prints one of:
#   JUST_UPGRADED <old> <new>       — marker found from a recent upgrade
#   UPGRADE_AVAILABLE <old> <new>   — remote VERSION differs from local
#   (nothing)                       — up to date, snoozed, disabled, or fetch failed
#
# The pexo-agent skill calls this opportunistically and relays UPGRADE_AVAILABLE
# to the user with the one-line update command. Failure is always silent.
#
# Usage:
...
#   SKILLS_STATE     — override ~/.pexo-skills state directory
#   REMOTE_URL       — override remote VERSION URL
set -euo pipefail

SKILL_DIR="${SKILL_DIR:-$(cd "$(dirname "$0")/.." && pwd)}"
STATE_DIR="${SKILLS_STATE:-$HOME/.pexo-skills}"
CACHE_FILE="$STATE_DIR/last-update-check"
MARKER_FILE="$STATE_DIR/just-upgraded-from"
SNOOZE_FILE="$STATE_DIR/update-snoozed"
VERSION_FILE="$SKILL_DIR/VERSION"
REMOTE_URL="${REMOTE_URL:-https://raw.githubusercontent.com/pexoai/pexo-skills/main/VERSION}"
